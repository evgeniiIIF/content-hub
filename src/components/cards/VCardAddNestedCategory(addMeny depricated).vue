  ...
  export default {
    ...
    methods: {
      ...
      onSubmitNestedCategory() {
        this.dataToSend = this.bodyItems.map((itemL1, index) => {
          let name, description;

          itemL1.forEach((itemL2) => {
            if (itemL2.name === 'name') name = itemL2.value;
            if (itemL2.name === 'description') description = itemL2.value;
          });

          // return {
          //   [`categories[${index}][parent_id]`]: this.parentItemData.id,
          //   [`categories[${index}][name]`]: name,
          //   [`categories[${index}][description]`]: description,
          // };
          return {
            parent_id: this.parentItemData.id,
            name: name,
            description: description,
          };
        });
        console.log(this.dataToSend);
        this.dataToSend.forEach((item) => this.SEND_NESTED_CATEGORY_DATA(item));

        // this.SEND_NESTED_CATEGORY_DATA(this.dataToSend);
      },
